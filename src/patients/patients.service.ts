...
export class PatientsService {
  ...
  async create(patientData: Partial<Patient>): Promise<Patient> {
    ...
    if (patientData.total_sessions && patientData.total_amount) {
      patientData.per_session_amount =
        patientData.total_amount / patientData.total_sessions;
    }
  ...
  async update(
    ...
    if (
      ...
      (updateData.total_sessions !== patient.total_sessions ||
        updateData.total_amount !== patient.total_amount)
    ) {
  ...
  async getStats(): Promise<{
    ...
    const active = await this.patientsRepository.count({
      where: { status: PatientStatus.ACTIVE },
    });
    const completed = await this.patientsRepository.count({
      where: { status: PatientStatus.COMPLETED },
    });
    const dropped = await this.patientsRepository.count({
      where: { status: PatientStatus.DROPPED },
    });
