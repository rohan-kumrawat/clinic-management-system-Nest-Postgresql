...
export class PdfService {
  ...
  private addHeader(doc: any, title: string) {
    ...
    doc
      ...
      .fontSize(12)
      .font('Helvetica')
      .text(this.clinicInfo.subName, 50, 60, { align: 'center' })
      .fontSize(10)
      .text(this.clinicInfo.tagline, 50, 75, { align: 'center' });
    ...
    doc.fontSize(10).text('SHASHIOTHERAPY', 55, boxY + 8);
    
    // Second box
    doc.rect(210, boxY, boxWidth, boxHeight).stroke();
    doc.fontSize(10).text('WEIGHT CULTIVATION', 215, boxY + 8);
    
    // Third box
    doc.rect(370, boxY, boxWidth, boxHeight).stroke();
    doc.fontSize(10).text('GENIAL HATLOH', 375, boxY + 8);
  ...
  private addFooter(doc: any) {
    ...
    doc.fontSize(9)
      .text('Clinic Address', 50, pageHeight - 60)
      .text(this.clinicInfo.clinicAddress, 50, pageHeight - 50)
      .text('Residence', 300, pageHeight - 60)
      .text(this.clinicInfo.residenceAddress, 300, pageHeight - 50);
    
    // Generation info and copyright
    doc.fontSize(8)
      .text(`Generated on: ${new Date().toLocaleString()}`, 50, pageHeight - 30, { align: 'center' })
      .text(`© ${new Date().getFullYear()} ${this.clinicInfo.name}. All rights reserved.`, 50, pageHeight - 20, { align: 'center' });
  }
  ...
  async generatePendingPaymentsReport(data: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        const totalPending = data.reduce((sum: number, patient: any) => sum + patient.pending_amount, 0);
        doc.fontSize(10)
          .text(`Total Patients with Pending Payments: ${data.length}`, 50, 230)
          .text(`Total Pending Amount: ${this.formatCurrency(totalPending)}`, 50, 250);
